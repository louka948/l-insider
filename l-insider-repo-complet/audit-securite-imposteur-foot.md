...
| Webhook Stripe non idempotent | Un événement Stripe renvoyé deux fois duplique la ligne `purchases` | Contrainte `unique` sur `stripe_payment_intent_id`/`stripe_subscription_id` + `on conflict do nothing` | `docs/schema-comptes-premium.sql` (nouvelle migration) + `supabase/functions/stripe-webhook/index.ts` |
